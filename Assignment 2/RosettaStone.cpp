...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
     ...
     */
    if(kGramLength<=0){
        error("invalid number of k");
    }
    Map<string,double> result = {};
    int len = str.size();
    for(int i=0;i<=len-kGramLength; i++){
        string target = str.substr(i, kGramLength);
        result[target] +=1;
    }
    return result;
}
...
//    handle errors
    if (numToKeep<0){
        error("invalid negtive k");
    } else if(numToKeep >= source.size()){
        return source;
    }
//    put every key and negtive values into th pq
    PriorityQueue<string> pq;
    for (string key: source.keys()){
        double negfre = -1*source[key];
        pq.enqueue(key,negfre);
    }
//    dequeue the smallest ks keys and add them to the result map
    Map<string,double> result = {};
    for(int i=0;i<numToKeep;i++){
        string ele = pq.dequeue();
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
     ...
     * this function.
     */
//    using a for loop to iterate the lhs and check all the shared keys with rhs
    double cossim = 0;
    for (string key: lhs.keys()){
        if(rhs.containsKey(key)){
            cossim = cossim +lhs[key]*rhs[key];
        }
    }
    return cossim;
}
...
//    calculate every cosineSimilarity bewteen textprofile with the ones in the corpora
//    and save the language name and similarity in a pq
    PriorityQueue<string> compareresult;
    for(Corpus lanPorfile: corpora){
        double cosSim = cosineSimilarityOf(textProfile,lanPorfile.profile);
        compareresult.enqueue(lanPorfile.name,-cosSim);
    }
    return compareresult.dequeue();
