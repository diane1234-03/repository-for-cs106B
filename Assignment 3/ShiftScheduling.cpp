...
double calculateValue(Set<Shift> chosenshifts){
    double sum = 0;
    for(Shift s: chosenshifts){
        sum += valueOf(s);
    }
    return sum;
}

bool overLapsOrnot(Shift curr, Set<Shift> chosenshifts){
    bool flag = false;
    for(Shift s: chosenshifts){
        flag = flag || overlapsWith(curr,s);
    }
    return flag;
}

Set<Shift> highestValueScheduleHelper(Set<Shift> shifts, int remainingHours, Set<Shift> chosenshifts) {
    ...
    shifts = shifts - curr;
//    without the shift
    Set<Shift> withoutCurrtShifts = highestValueScheduleHelper(shifts, remainingHours,chosenshifts);
    int withoutValue = calculateValue(withoutCurrtShifts);
//    with the shift
    if(!overLapsOrnot(curr,chosenshifts)){
        chosenshifts.add(curr);
        Set<Shift> withCurrtShifts = highestValueScheduleHelper(shifts, remainingHours-lengthOf(curr),chosenshifts);
        int withValue = calculateValue(withCurrtShifts);
        if(withoutValue< withValue){
            return withCurrtShifts;
        }
    }
    return withoutCurrtShifts;
}
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    Set<Shift> result = highestValueScheduleHelper(shifts, maxHours, chosenshifts);
    return result;
}
