...
string joint(Vector<string> tokens) {
    string s="";
...
Set<string> allEmphasesOf(const string& sentence) {
    ...
    Set<string> result;
    if(tokens.size()== 1){
        result.add(toUpperCase(tokens[0]));
        result.add(toLowerCase(tokens[0]));
    } else {
        string curr = tokens[0];
        tokens.remove(0);
        string leftSen = joint(tokens);
        Set<string> previous = allEmphasesOf(leftSen);
        for(string preRes: previous){
            string temp1 = toUpperCase(curr) + preRes;
            string temp2 = toLowerCase(curr) + preRes;
            result.add(temp1);
            result.add(temp2);
        }
    }
