...
//Set<Set<Pair>> MatchingHelper(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
//    /* TODO: Delete this comment and these remaining lines, then implement this function. */
//    if(possibleLinks.isEmpty()){
//        return {matching}
//    }
//    string randomKey = possibleLinks.firstKey();
//    Set<string> partners = possibleLinks[randomKey];
...
//    basecase if the map is empty then it is true
    if(possibleLinks.isEmpty()){
        return true;
    }
//    basecase if the map has a empty value for a key then it is false
    for(string name: possibleLinks.keys()){
        if(possibleLinks.get(name).isEmpty()){
            return false;
        }
    }

//    pick a randomkey to start
    string randomKey = possibleLinks.firstKey();
    Set<string> partners = possibleLinks[randomKey];
    bool uiltflag = false;
    Set<Pair> curr = {};
    for(string p: partners){
        ...
        newpossibleLinks.remove(randomKey);
        newpossibleLinks.remove(p);
        for(string name: newpossibleLinks.keys()){
            newpossibleLinks[name].remove(p);
            newpossibleLinks[name].remove(randomKey);
        }
        bool flag = hasPerfectMatching(newpossibleLinks,matching);
        if(flag){
            Pair a(randomKey,p);
            curr.add(a);
        }
        uiltflag = uiltflag || flag;
    }
    if(!curr.isEmpty()){
        matching.add(curr.first());
    }
...
PROVIDED_TEST("hasPerfectMatching works on a square of people, and produces output.") {
    ...
    Set<Pair> matching;
//    hasPerfectMatching(links, matching);
