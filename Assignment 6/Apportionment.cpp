...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    for(string key: populations.keys()){
        result[key] = 1;
    }
    numSeats = numSeats - populations.size();
    HeapPQueue pq = {};
    for(string key: populations.keys()){
        DataPoint newdp = {key, populations[key]/sqrt(result[key]*(result[key]+1))*-1};
        pq.enqueue(newdp);
    }
    while (numSeats>0) {
        DataPoint largest = pq.dequeue();
        result[largest.name] +=1;
        string key = largest.name;
        largest.weight = populations[key]/sqrt(result[key]*(result[key]+1))*-1;
        numSeats -=1;
        pq.enqueue(largest);
    }
    return result;
