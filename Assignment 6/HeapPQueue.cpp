...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    logicalSize +=1;
    int curpos = logicalSize;
    elems[curpos] = data;
    int childPos = logicalSize/2;
    while (curpos>1 & elems[childPos].weight >elems[curpos].weight) {
        elems[curpos] = elems[childPos];
        elems[childPos] = data;
        curpos = childPos;
        childPos = curpos/2;
    }
...
DataPoint HeapPQueue::dequeue() {
    ...
    logicalSize--;
    int curr = 1;
    int lowerChild = lowerWeightChild(curr);
    while (lowerChild >0 & elems[curr].weight> elems[lowerChild].weight) {
        DataPoint temp = elems[curr];
        elems[curr] = elems[lowerChild];
        elems[lowerChild] = temp;
        curr = lowerChild;
        lowerChild = lowerWeightChild(curr);
    }
    return result;
}
...
int HeapPQueue::lowerWeightChild(int curr){
