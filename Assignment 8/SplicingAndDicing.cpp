...
Nucleotide* toStrand(const string& str) {
    ...
    result->prev = nullptr;
    result->value = str[0];
    string news = str.substr(1,str.size()-1);
    Nucleotide* curr = result;
    while(news != ""){
        ...
        curr->next = next;
        curr = next;
        news = news.substr(1,news.size()-1);
    }
    curr->next = nullptr;
    return result;
}
 ...
 */
bool check(Nucleotide* dna, Nucleotide* target){
...
//        return dna;
//    }
//    return findFirst(dna->next,target);
//}
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    while(dna != nullptr && !check(dna,target) ){
        dna = dna->next;
    }
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    Nucleotide* startp = findFirst(dna,target);
    if(dna == nullptr){
        return true;
    }
    ...
    Nucleotide* pre = startp->prev;
    Nucleotide* curr = startp;
...
//    /* We deliberately don't use EXPECT_EQUAL here because if this test
//     * fails, we don't
...
PROVIDED_TEST("spliceFirst works in the middle of a strand. studenttest") {
    Nucleotide* dna    = toStrand("ATGG");
    Nucleotide* target = toStrand("TG");

    /* Remove the sequence. Make sure that we didn't change where DNA was
     * pointing.
     */
    Nucleotide* originalDNA = dna;
    bool r = spliceFirst(dna, target);
    EXPECT(r);

    /* Confirm the link structure is still good. */
    bool l = isLinkedInStrand(dna);
    EXPECT(l);

    EXPECT_EQUAL(fromDNA(dna), "AG");
    EXPECT_EQUAL(dna, originalDNA);
