...
//    static all the frequence of each char
    ...
    if(charFreq.size()<=1){
        error("invalid str content");
    }

//    add every item into the pq
    PriorityQueue<EncodingTreeNode*> pq = {};
    for(char k: charFreq.keys()){
        EncodingTreeNode* newNode = new EncodingTreeNode;
        newNode->one = nullptr;
        newNode->zero = nullptr;
        newNode->ch = k;
        pq.enqueue(newNode,charFreq[k]);
    }

    while (pq.size()>1) {
        EncodingTreeNode* newTree = new EncodingTreeNode;
        newTree->ch = ' ';
        double prorityOfNew = pq.peekPriority();
        newTree->zero = pq.dequeue();
        prorityOfNew += pq.peekPriority();
        newTree->one = pq.dequeue();
        pq.enqueue(newTree,prorityOfNew);
    }
...
char decodeHelper(Queue<Bit>& bits, EncodingTreeNode* tree){
    if(isLeaf(tree)){
        return tree->ch;
    } else{
        Bit b = bits.dequeue();
        if(b == 0){
            return decodeHelper(bits,tree->zero);
        } else {
            return decodeHelper(bits,tree->one);
        }
    }
}
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
//    turn a queue of bits to string text, accroding to the given tree
    string result = "";
    while(!bits.isEmpty()){
        char r = decodeHelper(bits,tree);
        result += r;
    }
//    deleteTree(tree);
    return result;
...
void huffmanTableHelper(Map<char, string>& rM,
                        string prefix, EncodingTreeNode* tree){
    if(isLeaf(tree)){
        rM.put(tree->ch, prefix);
        return;
    }
    string left = prefix+"0";
    huffmanTableHelper(rM,left,tree->zero);
    string right = prefix+'1';
    huffmanTableHelper(rM,right,tree->one);
}

Map<char, string> huffmanTable(EncodingTreeNode* tree){
    string start="";
    Map<char, string> resultMap= {};
    huffmanTableHelper(resultMap,start,tree);
    return resultMap;
}
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    /* TODO: Delete this comment and the next few lines, then implement this. */
    Map<char, string> huffmancodes = huffmanTable(tree);
    Queue<Bit> result = {};
    for(char c:str){
        string hc = huffmancodes[c];
        for(char b:hc){
            if(b == '0'){
                result.enqueue(0);
            }else{
...
PROVIDED_TEST("Can compress a sample file.") {
    HuffmanResult file = {
        { 1, 1, 0, 0, 0},
        { 'M', 'D', 'C', },
        { 0,0,  0,1, 1}
    ...
    string r = decompress(file);
    EXPECT_EQUAL(r, "MDC");
}
PROVIDED_TEST("Can decompress a sample file.") {
    HuffmanResult file = {
        { 1, 1, 1, 0, 0, 0, 1, 0, 0},
        { 'M', 'D', 'C', 'L', 'V'},
...
//    EncodingTreeNode* tree = decodeTree(file.treeBits,file.treeLeaves);
//    string r = decodeText(file.messageBits,tree);
//    EXPECT_EQUAL(r, "MDCVL");
    EXPECT_EQUAL(decompress(file), "MDCVL");
}
